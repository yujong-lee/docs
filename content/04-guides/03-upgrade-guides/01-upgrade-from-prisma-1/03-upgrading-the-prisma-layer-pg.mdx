---
title: 'Upgrading the Prisma layer (PG)'
metaTitle: 'Upgrading the Prisma layer to Prisma 2'
metaDescription: 'Learn how to the Prisma layer to Prisma 2 and create your Prisma schema.'
---
...
1. Use the [Prisma Upgrade CLI](./how-to-upgrade#prisma-upgrade-cli) to resolve the [schema incompatibilities](./schema-incompatibilities) in the new Prisma 2 data model
...
The Prisma 2 CLI is available as the [`@prisma/cli`](https://www.npmjs.com/package/@prisma/cli) package on npm and is invoked via the `prisma` command.
...
With Prisma 2, the HTTP layer isn't exposed any more and Prisma Client 2.0 is configured to run requests "directly" against the database (that is, requests are proxied by Prisma's [query engine](../../../reference/tools-and-interfaces/prisma-client/query-engine), but there isn't an extra server any more).
...
Run the following SQL statements against your database:
  ...
  https://pris.ly/d/schema-incompatibilities#default-values-arent-represented-in-database
    ...
    ALTER TABLE "public"."Post" ALTER COLUMN "published" SET DEFAULT false;
  ...
  https://pris.ly/d/schema-incompatibilities#inline-1-1-relations-are-recognized-as-1-n-missing-unique-constraint
    ...
    ALTER TABLE "public"."Profile" ADD UNIQUE ("user");


  Migrate IDs from varchar(25) to varchar(30)
  https://pris.ly/d/schema-incompatibilities#mismatching-cuid-length

    ALTER TABLE "public"."Category" ALTER COLUMN "id" SET DATA TYPE character varying(30);
    ALTER TABLE "public"."Post" ALTER COLUMN "id" SET DATA TYPE character varying(30);
    ALTER TABLE "public"."Profile" ALTER COLUMN "id" SET DATA TYPE character varying(30);
    ALTER TABLE "public"."Profile" ALTER COLUMN "user" SET DATA TYPE character varying(30);
    ALTER TABLE "public"."User" ALTER COLUMN "id" SET DATA TYPE character varying(30);
...
The shown SQL statements are categorized into a number of "buckets", each aiming to resolve a certain [schema incompatibility](./schema-incompatibilities):
...
First, the Upgrade CLI helps you resolve the issue that [default values aren't represented in the database](./schema-incompatibilities#default-values-arent-represented-in-database) by generating SQL statements that add the respective `DEFAULT` constraints directly to the database.
...
#### 5.1.2. Replicate `@createdAt` behavior in Prisma 2

The next thing the tools does is help you resolve the issue that the behavior of [`@createdAt` isn't represented in database](./schema-incompatibilities#default-values-arent-represented-in-database)
...
#### 5.1.4. Fix mismatch of CUID length

> **Note**: These SQL statements will keep appearing in the Upgrade CLI even after you have changed the column types in the underlying database. This is currently a limitation in the Upgrade CLI.

Finally, the tool will help you [turn the current ID columns of type `VARCHAR(25)` into `VARCHAR(30)`](./schema-incompatibilities#mismatching-cuid-length) by adding a `UNIQUE` constraint to the foreign key column called `user` (named after the relation field in the Prisma 1 datamodel) in the database.
...
#### 5.1.5. Breaking changes detected
...
#### 5.2.1. Fix columns with ENUM data types
...
Now, the Upgrade CLI helps you fix all 1-1 and 1-n relations that Prisma 1 represents with relation tables and that [currently only exist as m-n relations](./schema-incompatibilities#all-non-inline-relations-are-recognized-as-m-n) in your new Prisma 2 schema. Concretely, this is the case for the `User` ↔ `Post` relation which currently is defined as m-n but _should_ really be a 1-n relation. 
...
In that step, the Upgrade CLI also fixes other issues that occured in the transition to Prisma 2:
...
One thing you'll notice with this version of the Prisma 2 schema is that all [relation fields](../../../reference/tools-and-interfaces/prisma-schema/relations#relation-fields) are named after their respective models, e.g:
...
There are a few schema incompatibilities that were not yet resolved by the Upgrade CLI. At this point you still haven't fixed [scalar lists](./schema-incompatibilities#scalar-lists-arrays-are-maintained-with-extra-table) and [cascading deletes](./schema-incompatibilities#cascading-deletes-are-not-supported-in-prisma-2). You can find the recommended workarounds for these on the [Schema incompatibilities](./schema-incompatibilities) page. 
...
Congratulations, you have now upgraded your Prisma layer to Prisma 2! From here on, you can move on to update your application code using on of the following guides:
...
This section contains a high-level and side-by-side comparison of the Prisma Client APIs of Prisma 1 and Prisma 2. For more details about the new Prisma Client API, you can explore the [Prisma Client](../../../reference/tools-and-interfaces/prisma-client) docs.
...
> **Note**: You can learn more about the new pagination API in the respective [release notes](https://github.com/prisma/prisma/releases/tag/2.0.0-beta.7) or the [Pagination](../../../reference/tools-and-interfaces/prisma-client/pagination) page in the docs.
...
In Prisma 1, the only ways to select specific fields and/or load relations of an object was by using the string-based `$fragment` and `$graphql` functions. With Prisma 2, this is now done in a clean and type-safe manner using [`select`](../../../reference/tools-and-interfaces/prisma-client/field-selection#select) and [`include`](../../../reference/tools-and-interfaces/prisma-client/field-selection#include).

Another benefit of this approach is that you can use `select` and `include` on _any_ Prisma Client query, e.g. `findOne`, `findMany`, `create`, `update`, `delete`, ...
