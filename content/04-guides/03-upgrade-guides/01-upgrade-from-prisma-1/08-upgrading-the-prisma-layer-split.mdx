---
title: 'Upgrading the Prisma layer (Split)'
metaTitle: 'Upgrading the Prisma layer to Prisma 2 (Split)'
...
The shown SQL statements are categorized into a number of "buckets", all aiming to resolve a certain [schema incompatibility](./schema-incompatibilities):
...
<h3>5.1.2. Add missing `DEFAULT` constraints to the database</h3>

Next, the Upgrade CLI helps you resolve the issue that [default values aren't represented in the database](./schema-incompatibilities#default-values-arent-represented-in-database) by generating SQL statements that add the respective `DEFAULT` constraints directly to the database.
...
<h3>5.1.3. Fix columns with JSON data types</h3>
...
<h3>5.1.4. Replicate `@createdAt` behavior in Prisma 2</h3>
...
<h3>5.1.5. Fix 1-1 relations by adding `UNIQUE` constraints</h3>
...
<h3>5.1.6. Fix mismatch of CUID length</h3>
...
<h3>5.1.7. Breaking changes detected</h3>
...
<h3>5.2. Fix the database schema via plain SQL (breaking)</h3>
...
<h3>5.2.1. Fix incorrect m-n relations</h3>
...
<h3>5.2. Re-introspect your database to update your Prisma schema</h3>
...
<h3>5.2. Add missing attributes to the Prisma 2 schema and other schema fixes</h3>
