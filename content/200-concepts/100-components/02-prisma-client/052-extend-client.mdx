...
Let's build a signup function that requires user and team information to create rows in your database. We'll build this function first in Javascript then in Typescript.
...
function computeFullName<User extends FullName>(
  user: User
): WithFullName<User> {
  return {
    ...user,
...
In the example above, we've defined a `User` generic that extends the `FullName` type, so it must contain a `firstName` and `lastName`. Finally, the return type tacks on a `fullName` attribute onto the `User` generic.
...
Once again, we'll start with Javascript so we can focus on the logic. You first want to create an exclude helper function that takes a user and returns the user without certain fields:
...
In example above, you've excluded the password by deleting that key off of the user.
